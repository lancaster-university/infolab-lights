...
return class SolarSystemEffect {
  constructor(display) {
    ...
    this.scaleFactor =
      ...
      (Math.log(planets[planets.length - 1].distance + 1) + 0.5);
    ...
    this.totalFramesInitialAnimation = 100; // 10 seconds at 10 fps
    this.totalFramesHoldToday = 300; // 30 seconds at 10 fps
    ...
    this.forwardAnimationRate = this.millisecondsPerYear * 1; // 1 year per second

    // Loop parameters
    this.loopStartDate = new Date('2000-01-01').getTime();

    // Sun's texture with rounded corners (same as before)
    this.sunSize = 9; // Size for the Sun
    this.sunTexture = [
      // ... [Sun texture remains unchanged]
    ...
    this.digitWidth = 5; // Each digit/letter is 5 pixels wide
    this.digitHeight = 7; // Each digit/letter is 7 pixels tall
