// Modified from Ben Simms' fireworks code, but with periodic colours pulled from the web
// Pulls data from the cheerlights.com project - see website for details
//
// Tweet "@cheerlights" on twitter with a colour to update every system plugged into the network :) 
...
class Firework {
   ...
   * @param {FireworksEffect} scene
   */
  static newRandom(scene, fixedColour) {
    ...
    let colour = fixedColour;
    if( colour === undefined )
      colour = hsvToRgb(Math.random(), 1.0, 1.0);
...
return class FireworksEffect {
  constructor(display) {
    ...
    this.cachedWebColour = undefined;
  ...
  syncWebRgb() {
    fetch( "https://api.thingspeak.com/channels/1417/field/2/last.json" )
      .then( ret => ret.json() )
      .then( ret => {
        this.cachedWebColour = ret
                            .field2
                            .substring(1)
                            .match(/.{1,2}/g)
                            .map( v => parseInt(v,16) );
